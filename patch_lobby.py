...
def patch():
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        const added = useGameStore.getState().addPlayer(payload.new as Player)
        if (added) {
          toast({ title: `${payload.new.nickname} joined the party!` })
        }
      })
    ...
    if old_subscriptions not in content:
        raise RuntimeError("Expected subscription block not found in src/app/lobby/[code]/page.tsx")
